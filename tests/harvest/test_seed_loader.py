...
def test_every_seed_row_loads(loaded) -> None:
    ...
    # 131 rows, three cross-domain pairs merged into one record each: the EU ETS
    ...
    assert result.confirmed + result.drafted == 128
...
def test_all_ten_domains_are_populated(loaded) -> None:
    ...
    assert sum(result.by_domain.values()) == 131, "the merged record counts under both domains"
