...
graph_app = typer.Typer(name="graph", help="The record store.", no_args_is_help=True)
seed_app = typer.Typer(name="seed", help="The curated fixture inventory.", no_args_is_help=True)
...
def _lines(lines: Iterable[str], *, as_json: bool) -> None:
    ...
    how it is consumed: a workflow step pipes the output straight into
    ``json.load``. Five commands printed their per-entry lines ahead of the
    payload whatever the flag said, so ``probe run --json`` and
    ``graph materialize --json`` emitted output that does not parse — silently,
    because nothing consumed them yet.
    """
    if as_json:
        return
    for line in lines:
...
def seed_load(
    ...
    """
    from datahub.fixtures.seed import load_seed
    from datahub.graph.loader import bootstrap
